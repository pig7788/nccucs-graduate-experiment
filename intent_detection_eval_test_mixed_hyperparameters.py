...
if __name__ == '__main__':
    ...
    BERT_COSINE_SIMILARITY_WEIGHT = args.bert_cosine_similarity_weight
    T5_STSB_SIMILARITY_WEIGHT = args.t5_stsb_similarity_weight
    INTENT_STRING_SIMILARITY_WEIGHT = args.intent_string_similarity_weight
    ...
    CONFUSION_MATRIX_CSV_FILE_NAME = '{}_{}_{}_{}_confusion_matrix.csv'.format(
        TGT_DATASET, EXECUTE_TYPE, DATA_TYPE, ENCODER_TYPE)
    HYPERPARAMETERS_RESULT_FILE_NAME = '{}_to_{}.txt'.format(
    ...
    HYPERPARAMETERS_RESULT_PATH = os.path.join(
        LOAD_MODEL_PATH, HYPERPARAMETERS_RESULT_FILE_NAME)
    ...
    if not os.path.isfile(HYPERPARAMETERS_RESULT_PATH):
        with open(HYPERPARAMETERS_RESULT_PATH, 'w') as writer:
            writer.write('{}{}'.format(
                '\t'.join(['bert_cosine_similarity_weight', 't5_stsb_similarity_weight',
                           'intent_string_similarity_weight', 'F1']),
                os.linesep))

    with open(HYPERPARAMETERS_RESULT_PATH, 'a') as writer:
        for ground_truth, prediction in all_normal_format_results:
            ground_truthes.append(ground_truth)
            predictions.append(prediction)

        f1_score_str = score_pattern.format(
            f1_score(ground_truthes, predictions, average='macro'))
        detailed_info = '\t'.join([BERT_COSINE_SIMILARITY_WEIGHT,
                                   T5_STSB_SIMILARITY_WEIGHT,
                                   INTENT_STRING_SIMILARITY_WEIGHT,
                                   f1_score_str])
        writer.write('{}{}'.format(detailed_info, os.linesep))
