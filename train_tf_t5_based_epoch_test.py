...
from utility import (T5Config, TFT5ForConditionalGeneration, argparse,
                     datetime, line_notifier, math, os, sys, tf, time,
                     timedelta, traceback)
...
if __name__ == "__main__":
    ...
    train_datasets = tf.data.Dataset.from_tensor_slices(train_tensors)
    ...
    datasets_total_length = int(
        round(train_datasets.cardinality().numpy() / BATCH_SIZE, 0))
    ...
    try:
        ...
        for epoch in range(1, EPOCHS + 1):
            ...
            if epoch in [3, 5, 7, 10, 13, 15, 17, 20, 23, 25, 27]:
                end = time.time()
